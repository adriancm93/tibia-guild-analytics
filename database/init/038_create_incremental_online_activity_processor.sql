-- 038_create_incremental_online_activity_processor.sql
-- Purpose:
--   Process online activity incrementally instead of rebuilding all online history.
--
-- Concept:
--   Each run finds unprocessed snapshot pairs, compares previous/latest statuses,
--   inserts interval rows, updates affected daily totals, updates affected all-time
--   totals, and marks those pairs as processed.

CREATE TABLE IF NOT EXISTS analytics.online_activity_processed_snapshot_pairs (
...
-- Seed processed tracking from interval rows we already calculated before this migration.
...
-- Supports incremental snapshot-pair comparison.
CREATE INDEX IF NOT EXISTS idx_guild_member_snapshot_world_guild_time_character
ON public.guild_member_snapshot (
    world,
    guild_name,
    extracted_at_utc,
    character_name
...
BEGIN
    ...
    -- Recalculate only affected character/day totals.
    ...
    -- Recalculate only affected all-time character totals.
